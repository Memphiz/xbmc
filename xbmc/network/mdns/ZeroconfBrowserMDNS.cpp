...
bool CZeroconfBrowserMDNS::doResolveService(CZeroconfBrowser::ZeroconfService& fr_service, double f_timeout)
{
  ...
  if (!fr_service.GetHostname().empty())
  {
    ...
    {
      CLog::Log(LOGWARNING, "ZeroconfBrowserMDNS: Could not resolve hostname %s - fallback to mdns resolveing", fr_service.GetHostname().c_str());

      // fallback to mdns resolveing
