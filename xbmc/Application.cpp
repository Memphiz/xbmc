...
bool CApplication::InitWindow()
{
  ...
  if (!g_Windowing.CreateNewWindow(CCompileInfo::GetAppName(), bFullScreen, CDisplaySettings::Get().GetResolutionInfo(RES_WINDOW), OnEvent))
  {
  ...
  if (!g_Windowing.CreateNewWindow(CCompileInfo::GetAppName(), bFullScreen, CDisplaySettings::Get().GetCurrentResolutionInfo(), OnEvent))
  {
    CLog::Log(LOGFATAL, "CApplication::Create: Unable to create window");
    return false;
  }
...
*/
  ...
  else
  {
     ...
     * separate locations. */
    if (!CFile::Exists(URIUtils::AddFileToFolder(appPath, "language")))
    {
...
bool CApplication::InitDirectoriesOSX()
{
  ...
  if (m_bPlatformDirectories)
  {
    ...
    #if defined(TARGET_DARWIN_IOS)
      std::string appName = CCompileInfo::GetAppName();
      CSpecialProtocol::SetHomePath(userHome + "/" + std::string(DarwinGetAppRootFolder()) + "/" + appName);
      CSpecialProtocol::SetMasterProfilePath(userHome + "/" + std::string(DarwinGetAppRootFolder()) + "/" + appName + "/userdata");
    #else
      std::string appName = CCompileInfo::GetAppName();
    ...
    #if defined(TARGET_DARWIN_IOS)
      CStdString strTempPath = URIUtils::AddFileToFolder(userHome,  std::string(DarwinGetAppRootFolder()) + "/" + appName + "/temp");
    #else
      CStdString strTempPath = URIUtils::AddFileToFolder(userHome, dotLowerAppName + "/");
    ...
    #if defined(TARGET_DARWIN_IOS)
      strTempPath = userHome + "/" + std::string(DarwinGetAppRootFolder());
    #else
